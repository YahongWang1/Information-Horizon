...
class LlavaMetaForCausalLM(ABC):
    ...
    def DivPrune(self, visual_feature_vectors, image_feature_length, cosine_matrix=None, threshold_ratio=0.1, num_save=0):            
        if num_save:
            threshold_terms = num_save
        else:
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        #divprune
        if 'PRUNED_LAYER_LIST' in os.environ:
        ...
        if 'PRUNED_LAYER_LIST' in os.environ:
            layer_str = os.environ.get("PRUNED_LAYER_LIST", "[]")
            layer_list = json.loads(layer_str)
            save_num_str = os.environ.get("SAVE_NUM_LIST", "[]")
            save_num_list = json.loads(save_num_str)
            if 0 in layer_list:
                method_pre = str(os.environ['METHOD_PRE'])
                SYS_TOKEN_LEN = 35 

                num_save = save_num_list[0]

                if method_pre == 'DivPrune':
                    diverse_ratio = float(num_save/img_feature_len)
                    cosine_matrix = None
                    if type(image_features) == list: #this is for LLaVA 1.6
                        img_feature_len = image_features[0].shape[0] #example is 2340x4096
                    else: #for LLaVa 1.5
                    ...
                    selected_visual_tokens, cosine_matrix = self.DivPrune(visual_tokens, img_feature_len,cosine_matrix,threshold_ratio=diverse_ratio, num_save=num_save)

                    if selected_visual_tokens.numel() > 0:
                        ...
                        keep_indexs = torch.cat((torch.arange(SYS_TOKEN_LEN,device=new_input_embeds.device), torch.arange(SYS_TOKEN_LEN+img_feature_len,new_input_embeds.shape[1],device=new_input_embeds.device)))
