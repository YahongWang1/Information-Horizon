...
# 两阶段剪枝配置：
# 格式：divprune_layer divprune_save random_layer random_save avg_tkn
# 192配置：layer0保留284，layer20随机保留38
# 64配置：layer0保留96，layer20随机保留10
configs=(
    "0 284 20 38 192"
    "0 96 20 10 64"
)
...
for i in $(seq 1 $seq_num); do
    for config in "${configs[@]}"; do
        # 解析配置：divprune_layer divprune_save random_layer random_save avg_tkn
        read -r divprune_layer divprune_save random_layer random_save avg_tkn <<< "$config"

        Model="$baseModel-DivPrune+Random-avg_tkn${avg_tkn}-DivPrune-K${divprune_layer}-save${divprune_save}-Random-K${random_layer}-save${random_save}-$i"

        export SAVE_NUM_LIST="[$divprune_save, $random_save]"
        export PRUNED_LAYER_LIST="[$divprune_layer, $random_layer]"
