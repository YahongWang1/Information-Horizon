...
# 两阶段剪枝配置：
# 格式：dart_layer dart_save random_layer random_save avg_tkn
# 192配置：layer2保留252，layer20随机保留38
# 64配置：layer1保留60，layer20随机保留27
configs=(
    "2 252 20 38 192"
    "1 60 20 27 64"
)
...
for i in $(seq 1 $seq_num); do
    for config in "${configs[@]}"; do
        # 解析配置：dart_layer dart_save random_layer random_save avg_tkn
        read -r dart_layer dart_save random_layer random_save avg_tkn<<< "$config"

        Model="$baseModel-DART+Random-avg_tkn${avg_tkn}-DART-K${dart_layer}-save${dart_save}-Random-K${random_layer}-save${random_save}-$i"

        export SAVE_NUM_LIST="[$dart_save, $random_save]"
        export PRUNED_LAYER_LIST="[$dart_layer, $random_layer]"
        export METHOD_LLM=DART
        export PRUNE_STG=random_prune

        # MME
        echo "DART Layer$dart_layer save$dart_save -> Random Layer$random_layer save$random_save"
